...
// The function that saves the city to our localStorage
var saveCity = (newCity) => {
var cityExists = false;
// Checks if City is in local storage
...
// If city is new, save to localStorage
if (cityExists === false) {
   localStorage.setItem('cities' + localStorage.length, newCity);
}
}

// Renders list of searched cities
var renderCities = () => {
$('#city-results').empty();
...
if (localStorage.length===0){
   if (lastCity){
       ...
       $('#search-input').attr("value", "Austin");
   }
} else {
...
// Get the current weather conditions (+ calling the 5 day forecast)
